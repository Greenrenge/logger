...
if (process.env.NODE_ENV === 'production') {
  ...
  log = (name) => {
    ...
    const log = (level, ...msgs) => {
      winstonConfig.log(level, `${template} : `, ...msgs)
    }
    ...
    if (!isEnabled(name)) {
      ...
      debug = (...msgs) => wrapCall(winstonConfig.debug, ...msgs)
    }

    return {
      log,
      silly: (...msgs) => wrapCall(winstonConfig.silly, ...msgs),
      debug, // depend on filter
      verbose: (...msgs) => wrapCall(winstonConfig.verbose, ...msgs),
      info: (...msgs) => wrapCall(winstonConfig.info, ...msgs),
      warn: (...msgs) => wrapCall(winstonConfig.warn, ...msgs),
      error: (...msgs) => wrapCall(winstonConfig.error, ...msgs)
    }
  }
} else {
